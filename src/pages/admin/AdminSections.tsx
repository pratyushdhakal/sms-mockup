...
export default function AdminSections() {
  const [sections, setSections] = useState<SectionEntry[]>(
    DATA_SECTIONS.map((s, i) => ({ id: `SEC${String(i + 1).padStart(3, "0")}`, name: s }))
  );
  ...
  function handleEdit(id: string) {
    ...
    setSections((prev) => prev.map((s) => s.id === id ? { ...s, name: editValue.trim() } : s));
    setEditingId(null);
    setEditValue("");
  }
                    ...
                    {editingId === s.id ? (
                      <div className="flex items-center gap-1">
                        <Input
                          ...
                          autoFocus
                        />
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-emerald-500" onClick={() => handleEdit(s.id)}>
                          <Check size={13} />
                        </Button>
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500" onClick={() => { setEditingId(null); setEditValue(""); }}>
                          <X size={13} />
                        </Button>
                      </div>
                    ) : (
